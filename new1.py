...
def background_task(arg):
    """ Example background task that takes an argument and prints something after a delay. """
    print(f"Task started with argument {arg}!")
    time.sleep(10)  # Simulating a time-consuming task
    print(f"Task completed with argument {arg}!")

@app.route('/start-task', methods=['POST'])
def start_task():
    arg = request.form.get('arg', '')  # Get an argument from the form data
    thread = Thread(target=background_task, args=(arg,))
